...
# Source Term Treatment {.unnumbered}
...
Let's look at the steady one-dimensional heat conduction example given in @Patankar2018 as given in equation \@ref(eq:2) where $k$ is the thermal conductivity, $T$ is the temperature, and $S$ is the rate of heat generation per unit volume.
...
\end{equation}
```
The discretization of equation \@ref(eq:2) can be done after utilizing the widely used grid-point notation where P is the central node, and E and W are the neighbors. More on the notation can be found at @Patankar2018.
...
## Explicit Treatment Su() {.unnumbered}
...
## Implicit Treatment Sp() {.unnumbered}
...
## Hybrid Treatment SuSp() {.unnumbered}
